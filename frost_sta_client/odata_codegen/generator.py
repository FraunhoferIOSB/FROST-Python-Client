...
def generate_from_metadata(xml_text: str, out_dir: str, module_name: str = "datamodel", source_url: str = "", odata_version: str = "") -> str:
    ...
    lines.append("from frost_sta_client.dao.base import BaseDao")
    lines.append("from frost_sta_client.model.ext.entity_type import EntityTypes")
    lines.append("")
    ...
    for fqn, et in model.get("entity_types", {}).items():
        ...
        lines.append(f"        meta = EntityTypes.get('{e_name}')")
        lines.append("        if meta is not None:")
        lines.append("            return BaseDao(service, meta)")
        lines.append("        # Fallback to generic OData DAO if available on the service")
        lines.append("        fn = getattr(service, '_generic_odata_dao_for_entity', None)")
        lines.append("        if callable(fn):")
        lines.append("            dao = fn(self)")
        lines.append("            if dao is not None:")
        ...
        # DAO accessor (BaseDao) for backward compatibility with service.create/update/patch/delete
        lines.append("    def get_dao(self, service):")
        lines.append(f"        return BaseDao(service, EntityTypes['{e_name}'])")
        lines.append("")
        ...
        for p in props:
            ...
            if _is_edm_datetimeoffset(inner_raw, model):
                ...
                if nullable:
                    ...
                    lines.append(f"        self._{sn} = value")
            lines.append("")
        # Specialized property overrides to match legacy wrapper behavior
        if e_name in ('ObservedProperty', 'Thing', 'Sensor', 'Datastream', 'Task', 'TaskingCapability'):
            lines.append("    @property")
            lines.append("    def properties(self):")
            lines.append("        return getattr(self, '_properties', None)")
            lines.append("")
            lines.append("    @properties.setter")
            lines.append("    def properties(self, value):")
            lines.append("        if value is None:")
            lines.append("            self._properties = None")
            lines.append("            return")
            lines.append("        if not isinstance(value, dict):")
            lines.append("            raise ValueError('properties should be of type dict!')")
            lines.append("        self._properties = value")
            lines.append("")
        if e_name == 'Observation':
            lines.append("    @property")
            lines.append("    def parameters(self):")
            lines.append("        return getattr(self, '_parameters', None)")
            lines.append("")
            lines.append("    @parameters.setter")
            lines.append("    def parameters(self, value):")
            lines.append("        if value is None:")
            lines.append("            self._parameters = None")
            lines.append("            return")
            lines.append("        if not isinstance(value, dict):")
            lines.append("            raise ValueError('parameters should be of type dict!')")
            lines.append("        self._parameters = value")
            lines.append("")
        if e_name in ('Task', 'TaskingCapability'):
            lines.append("    @property")
            lines.append("    def tasking_parameters(self):")
            lines.append("        return getattr(self, '_tasking_parameters', None)")
            lines.append("")
            lines.append("    @tasking_parameters.setter")
            lines.append("    def tasking_parameters(self, value):")
            lines.append("        if value is None:")
            lines.append("            self._tasking_parameters = None")
            lines.append("            return")
            lines.append("        if not isinstance(value, dict):")
            lines.append("            raise ValueError('tasking_parameters should be of type dict!')")
            lines.append("        self._tasking_parameters = value")
            lines.append("")
        if e_name == 'FeatureOfInterest':
            lines.append("    @property")
            lines.append("    def feature(self):")
            lines.append("        return getattr(self, '_feature', None)")
            lines.append("")
            lines.append("    @feature.setter")
            lines.append("    def feature(self, value):")
            lines.append("        if value is None:")
            lines.append("            self._feature = None")
            lines.append("            return")
            lines.append("        try:")
            lines.append("            self._feature = utils.geometry_to_json(value)")
            lines.append("        except ValueError:")
            lines.append("            # Legacy behavior expects TypeError for unsupported types")
            lines.append("            raise TypeError('feature should be a valid GeoJSON geometry')")
            lines.append("")
        if e_name == 'MultiDatastream':
            lines.append("    @property")
            lines.append("    def observed_area(self):")
            lines.append("        return getattr(self, '_observed_area', None)")
            lines.append("")
            lines.append("    @observed_area.setter")
            lines.append("    def observed_area(self, value):")
            lines.append("        if value is None:")
            lines.append("            self._observed_area = None")
            lines.append("            return")
            lines.append("        try:")
            lines.append("            self._observed_area = utils.geometry_to_json(value)")
            lines.append("        except ValueError:")
            lines.append("            raise ValueError('observed_area should be a valid GeoJSON geometry')")
            lines.append("")
            lines.append("    @property")
            lines.append("    def multi_observation_data_types(self):")
            lines.append("        return getattr(self, '_multi_observation_data_types', None)")
            lines.append("")
            lines.append("    @multi_observation_data_types.setter")
            lines.append("    def multi_observation_data_types(self, value):")
            lines.append("        if value is None:")
            lines.append("            self._multi_observation_data_types = None")
            lines.append("            return")
            lines.append("        if isinstance(value, str):")
            lines.append("            self._multi_observation_data_types = value")
            lines.append("            return")
            lines.append("        if isinstance(value, list) and all(isinstance(x, str) for x in value):")
            lines.append("            self._multi_observation_data_types = value")
            lines.append("            return")
            lines.append("        raise ValueError('multi_observation_data_types should be str or list[str]!')")
            lines.append("")
        ...
        lines.append("")
        # Navigation accessors (compatibility helpers)
        for np in navs:
            sn_nav = snake(np['name'])
            lines.append(f"    def get_{sn_nav}(self):")
            lines.append(f"        if self.service is None:")
            lines.append(f"            raise ValueError('service must be set on entity to access navigation collections')")
            lines.append(f"        result = self.service.{sn_nav}()")
            lines.append(f"        result.parent = self")
        ...
        for np in navs:
            ...
            if np.get("collection", False):
                ...
                lines.append(f"        if self.{sn} is not None:")
                lines.append(f"            data['{on}'] = self.{sn}.__getstate__()")
        # Drop empty Properties dict for serialization compatibility
        lines.append("        if isinstance(data.get('Properties', None), dict) and not data['Properties']:")
        lines.append("            del data['Properties']")
